...
make_tworout <- function() {
    ...
    enames <- c("a_r1", "b_r1", "c_r1", "d_r1", "e_r2", "f_r2", "g_r2", "h_r2", "r1_a", "r1_b", "r1_c", "r1_d", "r2_e", "r2_f", "r2_g", "r2_h", "r1_r2", "r2_r1")
    r1r2 <- list("e", "f", "g", "h")
    r2r1 <- list("a", "b", "c", "d")
    ...
    n2r <- network.edgelist(elist, n2r, ignore.eval=F, names.eval=enames)
...
make_AYn2r <- function() {
    n2r <- make_tworout()
    A <- make_A(n2r)
    Y <- as.matrix(read.table("Yinit.txt"))
    Y <- complete_Y(Y)
    A <- A[-c(12,16),]
    Y <- Y[-c(12,16),]
    return(list(A=A, Y=Y))
...
getod <- function(px, perm) {
    vnames <- c("a", "b", "c", "d", "e", "f", "g", "h")
    numv <- length(vnames)
    x <- perm[px]

    i <- floor((x-1)/numv)
    j <- (x-1) - i*numv
    odname <- paste(vnames[i+1], "_", vnames[j+1], sep="")
    return(odname)
}

cek_Y <- function(A, Y, perm) {
    for (i in 1:ncol(Y)) {
        red <- reduceA(A, Y[,i], perm, 1)
        if (any(red$y != Y[,i])) {
            cat("Y[",i,"] = (", c(Y[i]), ")\n")
            cat("red$y = (", red$y, ")\n")
        }
...
numinpos <- function(A,Y,perm) {
    ...
    n38 <- 0
    for (i in 1:N) {
        lpbox <- getlpbox(A,Y[,i],perm)
        nminpos[i] <- length(which(lpbox$xbd[1,] > 0))
        rangex5[i] <- lpbox$xbd[2,1] - lpbox$xbd[1,1]
        rangex24[i] <- lpbox$xbd[2,24] - lpbox$xbd[1,24]
        maxx5[i] <- lpbox$xbd[2,1]
        maxx24[i] <- lpbox$xbd[2,24]
        maxmin[i] <- max(lpbox$xbd[1,])
        if (lpbox$xbd[1,24] > 0) {
            n24 <- n24+1
        }
        if (lpbox$xbd[1,38] > 0) {
    ...
    return(list(nmp=nminpos, rx5=rangex5, rx24=rangex24, mx5=maxx5, mx24=maxx24, mm=maxmin, n24=n24, n38=n38))
}

get_yinit <- function(y) {
    y12 <- sum(y[1:4]) + y[16] - (sum(y[9:11]) + y[15])
    y16 <- sum(y[5:8]) + y[15] - (sum(y[12:14]) + y[16])
    y <- matrix(c(y[1:11], y12, y[12:14], y16, y[15:16]), ncol=1)
    return(y)
}
